...
int baronCase(int choice1, struct gameState *state, int currentPlayer){
  ...
  if (choice1){                                                 //Boolean if statement to assess whether player wants to discard an estate card
    ...
    while(card_not_discarded){
	    ...
	    else if (p >= state->handCount[currentPlayer]){           //BUG ADDED
	      if(DEBUG) {
...
int minionCase(int choice1, int choice2, struct gameState *state, int handPos, int currentPlayer){
  ...
  if (choice1){		                                              //Boolean if statement to assess the current player's card choice
	  state->coins = 2;                                           //BUG ADDED          
	}		
  else if (choice2){		                                        //Discard hand and redraw 4. Other players with 5+ cards discard hand and draw 4
	  //Discard hand
...
int ambassadorCase(int choice1, int choice2, struct gameState *state, int handPos, int currentPlayer){
  ...
  if (choice2 > 2 && choice2 < 0){                              //BUG ADDED                  
	  return -1;				
	}
...
int mineCase(int choice1, int choice2, struct gameState *state, int handPos, int currentPlayer){
  ...
  if ((getCost(state->hand[currentPlayer][choice1]) + 3) >= getCost(choice2)){  //BUG ADDED
	  return -1;
	}
  ...
  for (i = 0; i < state->handCount[currentPlayer]; i++){
	  if (state->hand[currentPlayer][i] == j){
	    discardCard(j, currentPlayer, state, 0);	              //BUG ADDED		
	    break;
	  }
	}
...
int cardEffect(int card, int choice1, int choice2, int choice3, struct gameState *state, int handPos, int *bonus)
{
  ...
  switch( card ) 
    ...
    case baron:
      return baronCase(choice1, state, currentPlayer);
